...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_times_both_or_neither') THEN
    ALTER TABLE public.atividades 
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_end_after_start') THEN
    ALTER TABLE public.atividades 
...
CREATE POLICY "atividades_insert_policy" ON public.atividades FOR INSERT TO authenticated WITH CHECK (is_admin());
CREATE POLICY "atividades_update_policy" ON public.atividades FOR UPDATE TO authenticated USING (is_admin()) WITH CHECK (is_admin());
CREATE POLICY "atividades_delete_policy" ON public.atividades FOR DELETE TO authenticated USING (is_admin());
